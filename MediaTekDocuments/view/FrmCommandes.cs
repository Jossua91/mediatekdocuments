...
namespace MediaTekDocuments.view
{
    ...
    public partial class FrmCommandes : Form
    {
        ...
        private void RemplirInformationsDetailleesLivre(Livre livre)
        {
            ...
            try
            {
                if (livre.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        byte[] data = client.DownloadData(livre.Image);
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbLCommandeLivreImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                {
                    pcbLCommandeLivreImage.Image = Image.FromFile(livre.Image);
                }
            }
            catch
            {
        ...
        private void RemplirInformationsDetailleesDvd(Dvd leDvd)
        {
            ...
            try
            {
                if (leDvd.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        byte[] data = client.DownloadData(leDvd.Image);
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbCommandeDvdImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                {
                    pcbCommandeDvdImage.Image = Image.FromFile(leDvd.Image);
                }
            }
            catch
            {
        ...
        public void RemplirInformationsRevue(Revue laRevue)
        {
            ...
            try
            {
                if (laRevue.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        byte[] data = client.DownloadData(laRevue.Image);
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbAbonnementImageRevue.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                {
                    pcbAbonnementImageRevue.Image = Image.FromFile(laRevue.Image);
                }
            }
            catch
            {
        ...
        private void dgvAbonnementParutions_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvAbonnementParutions.CurrentCell != null)
            {
                ...
                try
                {
                    if (exemplaire.Photo.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                    {
                        using (WebClient client = new WebClient())
                        {
                            byte[] data = client.DownloadData(exemplaire.Photo);
                            using (MemoryStream stream = new MemoryStream(data))
                            {
                                pcbAbonnementImageExemplaire.Image = Image.FromStream(stream);
                            }
                        }
                    }
                    else
                    {
                        pcbAbonnementImageExemplaire.Image = Image.FromFile(exemplaire.Photo);
                    }
                }
                catch
                {
