...

{
    ...
    public partial class FrmMediatek : Form
    {
            ...
            try
            {
                if (livre.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        ...
                        {
                            pcbLivresImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
        ...
        private void AfficheDvdInfos(Dvd dvd)
        {
            ...
            try
            {
                if (dvd.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        byte[] data = client.DownloadData(dvd.Image);
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbDvdImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
        ...
        private void AfficheRevuesInfos(Revue revue)
        {
            ...
            try
            {
                if (revue.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        byte[] data = client.DownloadData(revue.Image);
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbRevuesImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                {
                    pcbRevuesImage.Image = Image.FromFile(revue.Image);
                }
            }
            catch
            {
            ...
            try
            {
                if (revue.Image.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbReceptionRevueImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                {
                    pcbReceptionRevueImage.Image = Image.FromFile(revue.Image);
                }
            }
            catch
            {
            ...
            try
            {
                if (filePath.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                {
                    using (WebClient client = new WebClient())
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            pcbReceptionExemplaireImage.Image = Image.FromStream(stream);
                        }
                    }
                }
                else
                ...
                try
                {
                    if (exemplaire.Photo.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                    {
                        using (WebClient client = new WebClient())
                        {
                            ...
                            using (MemoryStream stream = new MemoryStream(data))
                            {
                                pcbReceptionExemplaireRevueImage.Image = Image.FromStream(stream);
                            }
                        }
                    }
                    else
